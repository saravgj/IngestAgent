...
import requests
import sys  # Import sys for flushing stdout, though we'll remove its usage in _call_gemini_api

# --- Trulens Imports ---
from trulens.core import TruSession
from trulens.apps.custom import TruCustomApp  # Corrected Import: Using trulens.app for TruCustomApp
from trulens.apps.custom import instrument  # Corrected Import: Using trulens.apps.custom for instrument
...
class IngestAgent:
    ...
    def __init__(self, config):
        ...
        logger.info("Ingest Agent instance fully initialized and ready.")
    ...
    def _simulate_ffprobe(self, file_path):
        """
    ...
    # Removed the @instrument decorator from _call_gemini_api
    def _call_gemini_api(self, prompt, schema=None):
        """
        ...
        apiKey = ""
        # Check if __app_id is defined and not empty, if so, assume Canvas provides the API key
        ...
        elif not apiKey:  # If apiKey is still empty at this point
            logger.warning(
        ...
        try:
            ...
            if result.get("candidates") and result["candidates"][0].get("content") and result["candidates"][0][
                ...
                logger.debug(f"LLM call made. Recording will be handled by TruCustomApp wrapper.")
    ...
    def _real_ai_enrichment(self, metadata, content_type="episode"):
        """
        ...
        # Make a copy to avoid modifying the original metadata if an error occurs
        ...
        logger.debug(f"Thought Chain: Attempting to generate AI summary based on prompt: '{prompt_summary[:70]}...'")
        print("DEBUG: Calling _call_gemini_api for summary generation.")
        sys.stdout.flush()
        try:  # Added try-except for individual LLM calls
            summary = self._call_gemini_api(prompt_summary)
        ...
        logger.debug(f"Thought Chain: Attempting to generate AI keywords based on prompt: '{prompt_keywords[:70]}...'")
        ...
        if "mood" not in enriched_metadata:
            ...
            logger.debug(f"Thought Chain: Attempting to generate AI mood based on prompt: '{prompt_mood[:70]}...'")
            print("DEBUG: Calling _call_gemini_api for mood generation.")
            sys.stdout.flush()
            try:  # Added try-except for individual LLM calls
                mood = self._call_gemini_api(prompt_mood)
    ...
    def _get_asset_type_from_path(self, file_path):
        ...
        if "ad" in os.path.basename(file_path).lower():
            ...
            return "trailer"
        logger.debug("Thought Chain: Defaulting to 'episode' based on filename heuristic.")
        return "episode"

    def asset_acquisition(self, source_path):
        """
        ...
        try:
            ...
            logger.debug(
                f"Thought Chain: Asset acquisition failed due to error: {e}. Justification: File system access issue.")
        logger.debug(f"Thought Chain: Asset acquisition complete. Found {len(new_assets)} new assets.")
        return new_assets

    def format_detection_and_validation(self, asset_path, metadata):
        """
    ...
    def metadata_extraction_and_enrichment(self, asset_path, ffprobe_data):
        """
        ...
        metadata = {
            ...
            "checksum_sha256": self._calculate_checksum(file_path=asset_path),
        ...
        if enriched_metadata_from_ai is None:  # Corrected from === None to is None
            logger.error("AI enrichment returned None. Using original metadata for further processing.")
            # If AI enrichment fails completely, use the metadata gathered so far
    ...
    def content_normalization(self, metadata):
        """
        ...
        asset_path = os.path.join(self.config["watch_folders"][0],
                                  original_file_name)  # Reconstruct path for os.replace
    ...
    def policy_compliance_check(self, metadata):
        """
        ...
        if duration is not None:
            if metadata["asset_type"] == "ad":
                ...
                if duration < self.config["policies"]["min_duration_seconds"]:
                    ...
                    escalate_to_human = True
                    logger.debug(
                        "Thought Chain: General content duration below minimum. Justification: Duration too short. Escalation triggered.")
                if duration > self.config["policies"]["max_duration_seconds"]:
                    ...
                    escalate_to_human = True
                    logger.debug(
                        "Thought Chain: General content duration exceeds maximum. Justification: Duration too long. Escalation triggered.")
        else:
        ...
        if not compliance_status:
            logger.warning(f"Policy compliance issues for {metadata.get('title', 'N/A')}. Flags: {compliance_flags}")
            logger.debug(
    ...
    def event_driven_hand_off(self, processed_asset_info):
        """
        ...
        if processed_asset_info.get("compliance_status") == "FAIL":
            logger.warning("Asset handed off with compliance failures. Downstream agents should handle accordingly.")
            logger.debug(
    ...
    # Removed the @instrument decorator from ingest_asset_pipeline
    def ingest_asset_pipeline(self, asset_path):
        """
        Orchestrates the entire ingestion pipeline for a single asset.
        Tracks ingest success rate and handoff latency.
        """
        print(f"DEBUG: Entering ingest_asset_pipeline (first line) for {asset_path}")  # Added very early print
        ...
        if os.path.exists(sidecar_path):
            try:
                ...
                logger.warning(f"Could not parse sidecar JSON {sidecar_path}: {e}")
                logger.debug(
                    f"Thought Chain: Failed to parse initial sidecar JSON. Justification: Syntax error. Proceeding without initial override flags.")
            except Exception as e:
                logger.warning(f"Error reading sidecar {sidecar_path}: {e}")
                logger.debug(
                    f"Thought Chain: Error reading initial sidecar. Justification: File access issue. Proceeding without initial override flags.")
        ...
        try:
            ...
            processed_asset_info.update({
                ...
                "transcode_needed": transcode_needed,  # This is the post-override value
                "initial_transcode_required": initial_transcode_required,  # New field
                "ffprobe_data": ffprobe_data
            })

            # Decision logic for status after format validation
            if not format_valid:
                if initial_transcode_required and initial_metadata.get("operator_override_transcode_needed") is True:
                    # It was initially invalid and needed transcode, but operator overrode.
                    # It should proceed as COMPLETED, but maybe with a warning or special status.
                    processed_asset_info["status"] = "COMPLETED_WITH_OVERRIDE"  # New status for override
                    logger.info(f"Ingestion for {asset_path} proceeds due to operator override despite format issues.")
                elif initial_transcode_required and not initial_metadata.get("operator_override_transcode_needed"):
                    # It was initially invalid and needed transcode, but no override.
                    processed_asset_info["status"] = "TRANSCODE_NEEDED"
                    logger.info(f"Ingestion for {asset_path} completed, but transcode is needed.")
                else:
                    # It was invalid and either didn't need transcode or couldn't be transcoded.
                    ...
                    handoff_end_time = time.time()
                    self.total_handoff_latency += (handoff_end_time - handoff_start_time)
                    self.handoff_count += 1
                    logger.debug(
                        "Thought Chain: Decision - Pipeline terminated due to unresolvable format validation failure.")
                    return processed_asset_info
            else:
                logger.info(f"Format validation successful for {asset_path}.")
                processed_asset_info["status"] = "IN_PROGRESS"  # Keep as IN_PROGRESS to continue pipeline
            ...
            # Ensure metadata_extraction_and_enrichment always returns a dictionary
            ...
            if "operator_override_transcode_needed" in initial_metadata:
                metadata["operator_override_transcode_needed"] = initial_metadata["operator_override_transcode_needed"]
                logger.debug(
            ...
            normalized_path, updated_metadata = self.content_normalization(metadata)  # Pass metadata directly
            ...
            # Final status setting based on all checks
            if processed_asset_info["status"] == "COMPLETED_WITH_OVERRIDE":
                if not compliance_status:
                    # Override allowed format, but still has other compliance issues
                ...
                if not compliance_status:
                    # Needs transcode AND has other compliance issues
                    processed_asset_info["status"] = "TRANSCODE_NEEDED_AND_FAILED_COMPLIANCE"
                    processed_asset_info["compliance_flags"].extend(compliance_flags)
                    processed_asset_info["escalate_to_human"] = True
            elif processed_asset_info["status"] == "IN_PROGRESS":  # Normal flow, no prior special status
                processed_asset_info["status"] = "COMPLETED" if compliance_status else "FAILED_COMPLIANCE"

            # If escalate_to_human was set by policy compliance, ensure it's reflected
            if escalate_to_human:
                processed_asset_info["escalate_to_human"] = True
            ...
            if processed_asset_info["status"] in ["COMPLETED", "COMPLETED_WITH_OVERRIDE",
                ...
                self.successful_ingests += 1  # Count as successful if it made it through, even with warnings/overrides
            ...
            self.handoff_count += 1
            logger.debug(
                f"Thought Chain: Pipeline terminated due to critical unhandled error: {e}. Justification: Unrecoverable exception occurred.")
            return processed_asset_info

    def _report_kpis(self):
        """
        Calculates and logs the current KPI metrics.
        """
        logger.info("\n--- Ingest Agent KPI Report ---")

        ingest_success_rate = (self.successful_ingests / self.total_ingests * 100) if self.total_ingests > 0 else 0
        logger.info(f"ingest_success_rate: {ingest_success_rate:.2f}% (Threshold: < 98%)")

        retry_count_avg = (self.retry_attempts / self.total_ingests) if self.total_ingests > 0 else 0
        logger.info(
            f"retry_count_avg: {retry_count_avg:.2f} (Threshold: > 2) - Note: Retry logic not fully implemented.")

        tool_usage_success_rate = (
                    self.successful_gemini_calls / self.total_gemini_calls * 100) if self.total_gemini_calls > 0 else 0
        logger.info(f"tool_usage_success_rate: {tool_usage_success_rate:.2f}% (Threshold: < 95%)")

        metadata_completeness = (
                    self.total_metadata_fields_present / self.total_metadata_fields_expected * 100) if self.total_metadata_fields_expected > 0 else 0
        logger.info(f"metadata_completeness: {metadata_completeness:.2f}% (Threshold: < 90%)")

        handoff_latency_avg = (self.total_handoff_latency / self.handoff_count) if self.handoff_count > 0 else 0
        logger.info(f"handoff_latency_avg: {handoff_latency_avg:.4f} sec (Threshold: > 10 sec)")

        logger.info(f"duplicate_asset_incidents: {self.duplicate_asset_count} (Threshold: > 1 per day)")
        logger.info("--- KPI Report End ---\n")

    def run(self, interval_seconds=5):
        """
        Starts the Ingest Agent monitoring loop.
        """
        ...
        while True:
            for folder in self.config["watch_folders"]:
                ...
                for asset_path in new_assets:
                    self.ingest_asset_pipeline(asset_path)
            time.sleep(interval_seconds)
            logger.debug(f"Thought Chain: Waiting for {interval_seconds} seconds before next scan.")


# --- Main execution block ---
if __name__ == "__main__":
    # This block is for direct execution of ingest_agent.py, not used when imported by orchestrator
    # For testing, you might still want to clear directories here
    ...
    # Removed default.sqlite cleanup from here, as it's now handled by pipeline_orchestrator.py
    # if os.path.exists("default.sqlite"):
    #     os.remove("default.sqlite")

    os.makedirs("./ingest_source", exist_ok=True)

    logger.info("--- Starting Duplicate File Uploads Test ---")
    ...
    # Tru initialized externally by orchestrator, not here for standalone runs
    # agent = IngestAgent(CONFIG)

    # For standalone ingest_agent.py runs, you'd need to initialize Tru here
    # For now, this block is primarily for testing individual agent logic
    # and not for full Trulens integration when run standalone.
    # If you run ingest_agent.py directly, you'd need:
    # from trulens_eval import Tru
    # tru = Tru()
    agent = IngestAgent(CONFIG)

    first_asset_detected = agent.asset_acquisition("./ingest_source")
    if first_asset_detected:
        agent.ingest_asset_pipeline(first_asset_detected[0])
    ...
    remaining_assets = agent.asset_acquisition("./ingest_source")
    for asset_path in remaining_assets:
        agent.ingest_asset_pipeline(asset_path)
    ...
    other_assets = agent.asset_acquisition("./ingest_source")
